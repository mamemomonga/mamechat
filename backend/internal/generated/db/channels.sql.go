...
const getChannelRepostGuard = `-- name: GetChannelRepostGuard :one
...
type GetChannelRepostGuardRow struct {
	NoConsecutivePosts bool        `json:"no_consecutive_posts"`
	LastUserID         pgtype.Int8 `json:"last_user_id"`
}

func (q *Queries) GetChannelRepostGuard(ctx context.Context, id int64) (GetChannelRepostGuardRow, error) {
	row := q.db.QueryRow(ctx, getChannelRepostGuard, id)
	var i GetChannelRepostGuardRow
	err := row.Scan(&i.NoConsecutivePosts, &i.LastUserID)
	return i, err
}
